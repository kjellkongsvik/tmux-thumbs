...
mod tests {
  ...
  #[test]
  fn match_color() {
    println!("{}foo", color::Fg(&*get_color("green")));
    println!("{}foo", color::Fg(color::Green));
  ...
  #[test]
  fn parse_rgb() {
    println!("{}foo", color::Fg(&*get_color("#1b1cbf")));
    println!("{}foo", color::Fg(color::Rgb(27, 28, 191)));
  ...
  fn parse_invalid_rgb() {
    println!("{}foo", color::Fg(&*get_color("#1b1cbj")));
  }

  #[test]
  #[should_panic]
  fn no_match_color() {
    println!("{}foo", color::Fg(&*get_color("wat")));
  }
}
