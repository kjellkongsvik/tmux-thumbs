...
impl fmt::Debug for Match<'_> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    write!(
      ...
      self.pattern,
      self.text,
      self.hint.clone().unwrap_or_else(|| "<undefined>".to_string())
    )
  }
}
...
impl<'a> State<'a> {
  pub const fn new(lines: &'a Vec<&'a str>, alphabet: &'a str, regexp: &'a Vec<&'a str>) -> Self {
    Self {
      lines,
      alphabet,
      regexp,
    ...
    for (index, line) in self.lines.iter().enumerate() {
      ...
      loop {
        ...
        let submatches = all_patterns
          .iter()
          .filter_map(|tuple| tuple.1.find_iter(chunk).nth(0).map(|m| (tuple.0, tuple.1.clone(), m)))
          .collect::<Vec<_>>();
    ...
    if unique {
      ...
      for mat in &mut matches {
        if let Some(previous_hint) = previous.get(mat.text) {
          ...
          previous.insert(mat.text, hint.clone());
          mat.hint = Some(hint);
        }
      }
    } else {
...
mod tests {
  ...
  #[test]
  fn match_paths() {
    let lines = split("L /f/b.l, lorem ../log/kern.log /c.f:8");
    let custom = [].to_vec();
    let results = State::new(&lines, "abcd", &custom).matches(false, false);

    assert_eq!(results.len(), 3);
    assert_eq!(results.get(0).unwrap().text, "/f/b.l");
    assert_eq!(results.get(1).unwrap().text, "../log/kern.log");
    assert_eq!(results.get(2).unwrap().text, "/c.f");
  }

  #[test]
  fn match_paths_line_nos() {
    let lines = split("/h/k.kdl:67:76 /h/k.kdl:6");
    let custom = [r"(?P<match>([.\w\-@$~\[\]]+)?(/[.\w\-@$\[\]]+)+(:\d+(:\d+)?|\(\d+(,\d+)?\))?)"].to_vec();
    let results = State::new(&lines, "abcd", &custom).matches(false, false);

    assert_eq!(results.len(), 2);
    assert_eq!(results.get(0).unwrap().text, "/h/k.kdl:67:76");
    assert_eq!(results.get(1).unwrap().text, "/h/k.kdl:6");
  }
